...
export function settingsSheet(){
  ...
  <div class="card sub"><div class="row between mb2"><span id="syncStatus" class="muted">${syncText()}</span>${SY.token?'<button class="btn sm" data-x="syncnow">Jetzt sichern</button>':''}</div>
    ...
    <button class="btn ghost wide mt2" data-x="undo">Letzte Wiederherstellung rückgängig</button>
  ...
  <button class="btn wide mt4" data-x="close">Schließen</button>`,{
    ...
    undo:async()=>{const b=await lastSafetyBackup();if(!b){toast('Kein Sicherungsstand vorhanden');return}
      if(!confirm2('Stand von '+new Date(b.at).toLocaleString('de-DE')+' wiederherstellen?'))return;
      const {replaceState,save:sv,rebuildBestsFull}=await import('../state.js');replaceState(b.data);rebuildBestsFull();sv();toast('Wiederhergestellt');closeSheet();rerender()},
