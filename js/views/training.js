...
import {sheet,closeSheet,toast,prompt2,confirm2,rerender,render,svgCheck,confirm2 as ask} from '../ui.js';
...
function active(){
  ...
  ${a.exercises.map((ex,ei)=>{const prev=lastSets(ex.name);return`<div class="card ex">
    <div class="row between top"><div class="grow"><div class="tags">${ex.main?'<span class="tag main">Main</span>':''}${ex.type?`<span class="tag">${ex.type}</span>`:''}${ex.sug?`<span class="sug">↑ +${String(ex.sug).replace('.',',')} kg</span>`:''}</div><div class="name">${esc(ex.name)}</div></div><button class="icon" data-a="note" data-i="${ei}">✎</button><button class="icon" data-a="rmex" data-i="${ei}">✕</button></div>
