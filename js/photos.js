...
export async function pushBlobs(t){const entries=[];for(const p of PH){if(p.synced)continue;const rec=await get(p.id);if(!rec||!rec.data)continue;const {gh}=await import('./sync.js');const b=await gh('/git/blobs',{method:'POST',body:JSON.stringify({content:rec.data,encoding:'base64'})});entries.push({path:'photos/'+p.id+'.jpg',mode:'100644',type:'blob',sha:b.sha});rec.synced=true;p.synced=true;p.remoteSha=b.sha;await put(rec)}
